...
%--------------------------------------------------------------------------
function angle2vector(angle_deg,model)
points = exp(1j.*deg2rad(angle_deg));
...
if upper(model) == "2D"
    for idx = 1:numel(points)
        ...
        text(real(points(idx)),imag(points(idx)),[num2str(idx) ':' num2str(angle_deg(idx)) '°']);
        hold on
    end
...
elseif upper(model) == "3D"
    for idx = 1:numel(points)
        ...
        text(idx,real(points(idx)),imag(points(idx)),[num2str(idx) ':' num2str(angle_deg(idx)) '°']);
        hold on
    end
